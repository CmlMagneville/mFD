...
* Fix bug:
  ...
  * Correct `tr.cont.scales()` if NAs and stop_if_NA = FALSE failing because column with NA was transformed while standandardising to be filled with NA (no pb before because FactoMineR::PCA function was then not running).
