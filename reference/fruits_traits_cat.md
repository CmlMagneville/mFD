...
- trait_type:
  ...
  a character giving the trait type (**O** for Ordinal trait, **N** for
  Nominal trait, **Q** for Quantitative trait, and **F** for Fuzzy-coded
  trait)

- fuzzy_name:

  a character giving the name of fuzzy-coded traits (i.e. `Use`) to
  which 'sub-traits' (i.e. `raw`, `pastry`, and `jam`) belongs
...
Traits in this dataset correspond to columns (traits) of the
`fruits_traits` dataset.
