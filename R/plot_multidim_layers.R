...
#' 
#' @param asb_sp_relatw a matrix gathering species relative weight in each
#' assemblage. It can be retrieved through the \code{\link{asb.sp.summary}}
#' 
...
#'   sp_filter <- mFD::sp.filter(asb_k          = c("basket_1"), 
...
#'            asb_sp_coord2D  = list(sp_faxes_coord_fruits_b1_2D),
...
#' Plot FDiv indice
#'
#' This function adds mean distance to center of gravity of vertices, points 
#' and vertices of 1:N assemblages on the background plot
#'
#' @param ggplot_bg a ggplot object of the plot background retrieved through
#' the \code{\link{background.plot}} function.
#' 
...
#' assemblage. It can be retrieved through the \code{\link{asb.sp.summary}}
#' 
#' @param asb_vertices_nD a list (with names as in asb_sp_coord2D) of vectors with 
#' names of species being vertices in n dimensions.
#' 
...
fdiv.plot <- function(ggplot_bg,
                      ...
                      asb_meanDtoG,
                      plot_sp = TRUE,
                      shape_sp, color_sp, fill_sp, size_sp,
                      shape_vert, color_vert, fill_vert, size_vert,
  ...
  if (plot_sp) {
    ggplot_fdiv <- sp.plot(ggplot_bg = ggplot_fdiv,
                           ...
                           size_sp = size_sp, size_vert = size_vert,
                           shape_sp = shape_sp,
                           color_sp = color_sp,
                           fill_sp = fill_sp,
                           shape_vert = shape_vert,
...
#' @param color_sp a R color name or an hexadecimal code referring to the color
#'  of species from the studied assemblage. Default: `color_sp = "#0072B2"`. 
#'  If several assemblages it should be a vector with names as in `asb_sp_coord2D`.
#'  
