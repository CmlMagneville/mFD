...
funct.space.plot <- function(sp_faxes_coord, faxes = NULL, name_file = NULL,
  ...
  if (!is.null(faxes) && is.null(faxes_nm)) faxes_nm <- faxes
  ...
  for (k in (1:plot_nb)) {
    ...
    if (plot_ch) {
      ...
      # plotting
      plot_k <- plot_k +
        ggplot2::geom_polygon(data = vert_coord_xy_sorted, colour = color_ch, 
                              fill = fill_ch, alpha = alpha_ch)
    }
    ...
    if (!is.null(plot_sp_nm)) {
      plot_k <- plot_k +
        ggrepel::geom_text_repel(x = sp_coord_plot[ , x],
                                 y = sp_coord_plot[ , y],
  ...
  if (plot_nb == 3) {
    patchwork_plots_all <- (panels[[1]] + plot_caption + panels[[2]] + 
                            panels[[3]]) +
  ...
  if (plot_nb == 6) {
    patchwork_plots_all <- (panels[[1]] + patchwork::plot_spacer() + 
                            plot_caption + panels[[2]] + panels[[3]] + 
