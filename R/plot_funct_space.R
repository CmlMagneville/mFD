...
#'  \code{faxes}).
#'
#' @param range_faxes_lim a vector with minimum and maximum values of axes. Note
...
#'
#' @param color_sp a R color name or an hexadecimal code referring to the colour
...
#'
#' @param shape_sp a numeric value referring to the shape of symbol used for
#'   species. Default: shape_sp = 21 (filled circle).
#'
#' @param size_sp a numeric value referring to the size of symbol for species.
#'   Default: size_sp = 1.
#'
...
#'   of the convex hull filled by the pool of species. Default: fill_ch =
#'   "white".
#'
...
#'   vertices if \code{plot_vertices} = TRUE. Default: color_vert =
#'   'blueviolet'.
#'
...
#'   vertices position if \code{plot_vertices} = TRUE. Default: shape_vert = 23
#'   (filled diamond).
#'   
...
#'@importFrom ggplot2 geom_polygon geom_point geom_rect scale_x_continuous  
#'@importFrom ggplot2 scale_color_manual scale_fill_manual scale_shape_manual 
#'@importFrom ggplot2 scale_y_continuous scale_size_manual
...
funct.space.plot <- function(sp_faxes_coord, faxes = NULL, name_file = NULL,
                             ...
                             color_bg = "grey95",
                             color_sp = "darkgreen", fill_sp = "white",  
                             shape_sp = 21, size_sp = 1,
                             plot_ch = TRUE,  color_ch = "black", 
                             fill_ch = "white", alpha_ch = 0.3,
                             plot_vertices = TRUE, color_vert = "blueviolet", 
                             fill_vert = "blueviolet", shape_vert = 23, 
                             size_vert = 1,
                             plot_sp_nm = NULL, nm_size = 3, nm_color = "black", 
                             nm_fontface = "plain",
                             check_input = TRUE) {
  ...
  if (plot_vertices == TRUE) {
    ...
    plot_caption <- plot_caption +
      ...
      ggplot2::geom_point (x = range_faxes_lim[1] + spread_faxes*0.1,
                           y = range_faxes_lim[2] - spread_faxes*0.5,
                           colour = color_sp, fill = fill_sp, 
                           shape = shape_sp, size = size_sp) +
  ...
  if(plot_nb == 1) {
    ...
    patchwork_plots_all <- patchwork_plots_all +
     patchwork::plot_annotation(
       title = "Position of species along functional axes",
