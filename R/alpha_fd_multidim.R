# Function to compute a set of alpha functional diversity indices
#
...
#' as.matrix(asb_sp_w), ind_vect = c("fdis", "fmpd", "fnnd", "feve", "fric", "fdiv", "fori", "fspe"),
...
alpha.fd.multidim <- function(sp_faxes_coord, asb_sp_w,
  ...
  ## create outputs with NULL objects so work even if not all ind computed:
  asb_sp_occ <- NULL
  asb_sp_relatw <- NULL
  ...
  # build a matrix to store indices values for each assemblage:
  if ("fide" %in% ind_vect) {
  ...
  # create a list to store vertices identities if fric or fdiv computed:
  vert_nm_all_asb <- list()
  
  # create a list to store coordinates of species per assemblage to use it...
  # ... in output so it can be then used in graphical functions:
  sp_coord_all_asb <- list()
  
  # create a list to store mst per assemblage if feve computed to use it...
  # ... in output so it can be then used in graphical functions...
  # ... created in general loop for graphical issue:
  mst_all_asb <- list()
  
  
  # create a list to store a vector for each assemblage with coordinates ...
  # ...of the gravity center of vertices if fdiv computed...
  # ... created in general loop for graphical issue:
  grav_center_vert_coord <- list()
  
  
  # create a list to store the mean distance of species of an assemblage to...
  # ... the gravity center of species present in the assemblage...
  # ... created in general loop for graphical issue:
  mean_dtogravcenter_all_asb <- list()
  
  
  # create a list to store the identity of the nearest neighbor (fnnd) for ...
  # ... each species in a given assemblage if fnnd is computed...
  # ... created in general loop for graphical issue:
  nm_nn_all_asb <- list()
  
  # create a matrix to store the distance to the nearest neighbour (fnnd) for ...
  # ... each species in a given assemblage if fnnd is computed...
  # ... created in general loop for graphical issue:
  ...
  # create a list to store names of nearest neighbor in the global pool ...
  # ... for each species of a given assemblage:
  nm_nn_global_pool <- list()
  
  # create an empty dataframe that will contain the distance to the nearest...
  # ... neighbor of the global pool for each species of the global pool...
  #... created in general loop for graphical issue:
  dist_nn_global_pool <- data.frame()
  
  # create an empty dataframe that will contain the distance to the...
  # centroid of the global pool for each species of the global pool...
  # ... created in general loop for graphical issue:
  special_sp_global_pool <- data.frame()
  
  # create of a matrix to store fide values used to compute fdis values:
  ...
  # compute distance between species in the global pool for scaling and ...
  # ... fori computation
  dist_sp <- as.matrix(stats::dist(sp_faxes_coord, method = "euclidean"))
  dist_sp[which(dist_sp == 0)] <- NA
  
  # retrieve a matrix with relative weight of species per assemblage to use it...
  # ... in output so it can be then used in graphical functions:
  relat_w_all_asb <- asb_sp_w
  relat_w_all_asb <- apply(relat_w_all_asb, 1, function(x) {x / sum(x)})
  
  # retrieve a matrix of presence/absence of species per assemblage:
  asb_sp_w <- as.matrix(asb_sp_w)
  asb_sp_summary <- asb.sp.summary(asb_sp_w)
  asb_sp_occ <- asb_sp_summary$asb_sp_occ
  
  # retrieve species richness per assemblage and add it to the indices matrix:
  ...
  ## compute indices for each assemblage:
  # loop on assemblages to compute if asked fdis, fmpd, fnnd, feve, fric, fdiv:
  for (n in (1:nrow(asb_sp_w))) {
    k <- rownames(asb_sp_w)[n]
    
    # retrieve information on species belonging to assemblage named k to...
    #... compute indices:
    ...
    # fill the matrix of species coordinates for all assemblages:
    sp_coord_all_asb[[paste0("sp_coord", sep = "_", k)]] <- sp_faxes_coord_k
    ...
    if ("fdis" %in% ind_vect) {
      ...
      if (scaling == TRUE) {
        # compute distance between species in the global pool for scaling:
    ...
    if ("fric" %in% ind_vect) {
      if (check.input == TRUE) {
        if (nrow(sp_faxes_coord_k) < ncol(sp_faxes_coord_k)) {
          ...
          It is not the case for", sep = " ", k, sep = ".",
                      "Remove this assemblage or decrease the number of functional axes.
      ...
      if (scaling == TRUE) {
        ...
        # ... with options = 'FA', to compute the general area of the ...
        # ... functional hull:
        fric_value <- fric$fric
        
        # if FRic value for this asb can be computed (no coplanearity):
        if (! is.na(fric_value)) {
        ...
        # if Fric value for this asb can not be computed (coplanearity):
        if (is.na(fric_value)) {
          fric_value <- NA
        }
      ...
      vert_nm_all_asb[[paste0("vert_nm",sep = "_", k)]] <- fric$vertices_nm
    }
    ...
    if ("fdiv" %in% ind_vect) {
      ...
      if (is.character(vert_nm)) {
        ...
        if (! "fric" %in% ind_vect) {
          vert_nm_all_asb[[paste0("vert_nm", sep = "_", k)]] <- vert_nm
        }
        # fill the list of gravity center coordinates for each assemblage:
        grav_center_vert_coord[[paste0("grav_center_vert_coord", sep = "_",
                                       k)]] <- fdiv$details$B_coord
        # fill the list of mean distance to gravity center of species present...
        # ... in each assemblage:
        mean_dtogravcenter_all_asb[[paste0(
          "mean_dist_to_sp_gravcent_asb", sep = "_",
    ...
    if ("fnnd" %in% ind_vect) {
      ...
      if (scaling == TRUE) {
        ...
        fnnd_value <- fnnd_value / (max(unlist(dist_nn), na.rm = TRUE))
        asb_ind_values[k, "fnnd"] <- fnnd_value
      }
      # fill the matrix with the fnnd value if no scaling:
      fnnd_value <- fnnd$fnnd
      asb_ind_values[k, "fnnd"] <- fnnd_value
      
      # retrieve data to use for outputs:
      # a list for nms (because can have several nn) and a df for distances (because only one nn dist):
    ...
    if ("fori" %in% ind_vect) {
      ...
      # compute the minimal distance to the nearest neighbor for each...
      ...
      # compute identity to the nearest neighbor in the gp for plots...
      ...
      # scale fori value and fill the matrix of indices if scaling:
      if (scaling == TRUE) {
    ...
    if ("fspe" %in% ind_vect) {
      ...
      # compute specialization of each species in the global pool i.e. ...
      # ... distances to the centroid of the global pool of species:
      # coordinates of the gravity center of the vertices:
      centroid_global_pool <- apply(sp_faxes_coord, 2, mean)
      special_sp_global_pool <- apply(sp_faxes_coord, 1, function(x) {
    ...
    print(paste0(k, sep = " ", "done ", round((n/nrow(asb_sp_w))*100,digits = 1),"%"))
  } # end loop on assemblages
  
  
  ## create a matrix linking fide values and other indices:
    ...
    return_list <- list(functional_diversity_indices = asb_ind_values_all,
                        details = list(asb_sp_occ = asb_sp_occ,
                                       ...
                                       grav_center_vert_coord_list =
                                         grav_center_vert_coord,
                                       mean_dtogravcenter_list =
                                         mean_dtogravcenter_all_asb,
                                       dtogravcenter_global_pool_list =
                                         special_sp_global_pool,
                                       nm_nn_global_pool_list =
                                         nm_nn_global_pool,
                                       dist_nn_global_pool_list = dist_nn_global_pool,
                                       nm_nn_asb_list = nm_nn_all_asb,
                                       dist_nn_asb_list = dist_nn_all_asb,
