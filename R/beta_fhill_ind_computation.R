...
beta.fd.hill <- function(asb_sp_w, sp_dist, q = c(0, 1, 2), 
  ...
  # ensuring species are in the same order in
  # (asb_sp_w)
  ...
  mat_res <- matrix(NA, asb_nb, asb_nb, dimnames = list(asb_nm, 
                                                        asb_nm))
  if (0 %in% q) {
  ...
  for (x in 1:asb_pairs_nb) {
    ...
    # computing core variables for the pair of
    # assemblages ---- notations as in Chao et al 2019,
    # page 16, bottom right (with p for +)
    
    # weights of species (rows) in the 2 assemblages
    # (columns) (nik, bottom right p16)
    x_nik <- t(asb_sp_w[asb_nm_x, ])
    
    # total weight of species in the 2 assemblages
    x_npp <- sum(x_nik)
    
    # total weight of each species among the 2
    # assemblages
    x_nip <- apply(x_nik, 1, sum)
    
    # keeping only weight and distance of species
    # present in pair of assemblages
    x_sp <- names(which(x_nip > 0))
    x_nip <- x_nip[x_sp]
    x_nik <- x_nik[x_sp, ]
    x_sp_dist <- dij_tau[x_sp, x_sp]
    
    # weight of functionally distinct group of species
    # (aik, ai+ and vi+)
    ...
    # computing alpha, gamma and beta diversity
    # according to levels of q ----
    
    # q=0 ----
    if (0 %in% q) 
    {
      
      # hence sum of species attribute contribution
      ...
      if (beta_type == "Sorensen") {
        x_beta_q0 <- (x_gamma_q0/x_malpha_q0) - 
          1
      }
    ...
    if (1 %in% q) 
    {
      ...
      # alpha diversity (eq 7b) with special case of
      # 0^0=0 hence sum of species attribute contribution
      # depends on their occurrence
      x_malpha_q1 <- 0.5 * exp((-1) * sum(x_sp_vip * 
                                            (x_sp_aik/x_npp) * log(x_sp_aik/x_npp), 
                                          na.rm = T))
      
      # gamma diversity (eq 6b)
      x_gamma_q1 <- exp((-1) * sum(x_sp_vip * 
                                     (x_sp_aip/x_npp) * log(x_sp_aip/x_npp)))
    ...
    if (2 %in% q) 
    {
      ...
      # alpha diversity (eq 7a) with special case of
      # 0^0=0 hence sum of species attribute contribution
      # depends on their occurrence
      x_malpha_q2 <- 0.5/(sum(x_sp_vip * 
                                ((x_sp_aik/x_npp)^2)))
      ...
      if (beta_type == "Jaccard") {
        x_beta_q2 <- (x_gamma_q2/x_malpha_q2) - 
          1
      }
  ...
  if (details_returned == TRUE) {
    res <- list(beta_fd_q = beta_fd_q, details = list(malpha_fd_q = malpha_fd_q, 
