...
#'
#' @param GE a numerical vector giving the IUCN status rank (DD = NA, LC = 0, NT
#'   = 1, VU = 2, EN = 3, CR = 4) or the IUCN extinction probability associated
#'   with each status, see Mooers et al.(2008) for example with DD = NA, LC = 0,
#'   NT = 0.1, VU = 0.4, EN = 0.666, CR = 0.999)
#'
...
fuse <- function(sp_dist, sp_faxes_coord, 
                 nb_NN = 5, GE, standGE = F) {
  
  if (!identical(row.names(as.matrix(sp_dist)), 
  ...
  uni_res <- get.indicator(sp_dist = as.matrix(sp_dist), 
                           nb_NN = nb_NN)
  uniqu <- uni_res$Average_uniqueness[, 
                                      "Mean"]
  
  if (standGE == TRUE) {
    GE <- as.vector(vegan::decostand(GE, 
                                     "range", na.rm = TRUE))
  }
  
  # FUSE metrics calculation
  FUn_std <- as.vector(vegan::decostand(uniqu, 
                                        "range"))
  FUGE <- log(1 + (FUn_std * GE))
  FSp_std <- as.vector(vegan::decostand(spe, 
                                        "range"))
  FSGE <- log(1 + (FSp_std * GE))
  FUS <- FUn_std + FSp_std
  FDist_std <- as.vector(vegan::decostand(Fdistinct, 
                                          "range"))
  FUSE <- stats::setNames(FUGE + FSGE, 
                          nm = nm)
  FUSE_alt <- log(1 + FUS * GE)
  FDGE <- log(1 + (FDist_std * GE))
  
  data.frame(cbind(FUSE, FUSE_alt, FUGE, 
                   FDGE, FSGE, FUn_std, FSp_std, FDist_std))
...
get.indicator <- function(sp_dist, nb_NN) {
  ...
  NN <- lapply(1:length(Res), function(i) {
    Res[[i]][[2]]
  })
  
  rownames(Res_mean_sd) <- names(NN) <- names(Res)
  list(Average_uniqueness = Res_mean_sd, 
