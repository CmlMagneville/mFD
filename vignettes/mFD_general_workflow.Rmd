...
The first thing to do before starting analyses is knowing your data. To do so, you must be able to characterize traits you are using. That is why *mFD* package needs a dataframe summarizing trait type for each trait (*ie* each column of the *sp_tr* dataframe)
  ...
  👉 the dataframe you need to build must be as followed with the same columns names (*fruits* dataset example)
...
```
  ...
  * *dist_metric* is a character string referring to the metric used to compute distances 👉 The package allows the user to choose the metric used to compute functional distance. Three metrics are available and **the metric must be chosen based on traits data**:
...
💡 The lower the quality metric is, the better the quality of your space is. Here, thanks to mad value, we can see that the 5D space is the best one. That is why the following of this tutorial will use this multidimensional space.
