...
```{r}
fruits_traits_summ$"tr_summary_non_fuzzy_list"    # Summary for non-fuzzy traits
```

<br />

```{r}
fruits_traits_summ$"tr_summary_fuzzy_list"        # Summary for fuzzy traits
```

<br />

```{r}
fruits_traits_summ$"tr_types"                     # Traits types for non-fuzzy traits
```
...
```{r, results = "hide"}
sp_dist_fruits <- mFD::funct.dist(sp_tr = fruits_traits,             # Species x Traits data frame
                           tr_cat       = fruits_traits_cat,         # Traits informations
...
```{r}
sp_dist_fruits                                                   # Output of the function mFD::funct.dist()
```
...
```{r}
fspaces_quality_fruits$"quality_fspaces"                # Quality metrics of spaces
```
...
# 6. Plot chosen functional space
...
## 7.1. Functional diversity indices based on Functional Entities
...
## 7.2. Functional alpha diversity indices in a multidimensional space
...
* `sp_faxes_coord` is the species coordinates matrix. This dataframe gathers **only** axis of the functional space you have chosen based on **step 4** (it is not the species data frame that is the raw output of the `mFD::quality.fspaces()` function (**step 4**))
...
## 7.3. Functional beta diversity indices based on multidimensional space

<br />

`mFD` package allows you to compute beta diversity indices for each assemblage pairs following [Villéger _et al._ 2013](https://onlinelibrary.wiley.com/doi/full/10.1111/geb.12021). For that we will use the
`mFD::beta.fd.multidim()` function.

<br />

👉 This function can compute two types of functional beta diversity indices, either
**Jaccard** or **Sorensen**.
...
**turnover**. 💡 The **turnover** component is the highest if there is no shared
