...
* a **data frame** summarizing traits values for each species called `sp_tr_fruits` in
this tutorial
...
knitr::kable(head(sp_tr_fruits),
             caption = "Species x traits data frame based on the **fruits** dataset")
```

<br />

* a **matrix** summarizing species assemblages called `asb_sp_w_fruits` in this
...
knitr::kable(as.data.frame(asb_sp_w_fruits[c(1:6), c(1:6)]), centering = TRUE,
...
knitr::kable(head(sp_tr_cat_fruits), caption = "Traits types based on **fruits & baskets** dataset")
```
...
in the `sp_tr_fruits` data frame)
...
The function `mFD::sp.tr.summary()` summarizes the `sp_tr_fruits` data frame and
...
```{r}
sp_tr_fruits_summ <- mFD::sp.tr.summary(tr_cat = sp_tr_cat_fruits,    # Traits informations
                                 sp_tr  = sp_tr_fruits)        # Species x Traits data frame
```

<br />

```{r}
sp_tr_fruits_summ$"tr_summary_non_fuzzy_list"    # Summary for non-fuzzy traits
```

<br />

```{r}
sp_tr_fruits_summ$"tr_summary_fuzzy_list"        # Summary for fuzzy traits
```

<br />

```{r}
sp_tr_fruits_summ$"tr_types"                     # Traits types for non-fuzzy traits
```

<br />

```{r}
sp_tr_fruits_summ$"mod_list"                     # Traits types for non-continuous and non-fuzzy traits
```
...
`mFD::asb.sp.summary()`. It summarizes the `asb_sp_w_fruits` matrix and returns a list
...
```{r}
asb_sp_fruits_summ <- mFD::asb.sp.summary(asb_sp_w = asb_sp_w_fruits)     # Assemblages x Species data frame
```
...
```{r}
asb_sp_fruits_summ$"tot_ab_all_sp"              # Species total biomass in all assemblages
```

<br />

```{r}
asb_sp_fruits_summ$"tot_ab_all_asb"             # Total biomass per assemblage
```

<br />

```{r}
asb_sp_fruits_summ$"sp_richn_all_asb"           # Species richness per assemblage
```

<br />

```{r}
asb_sp_fruits_summ$"sp_nm_asb"[[1]]             # Names of species present in the first assemblage
```
...
```{r, results = "hide"}
sp_dist_fruits <- mFD::funct.dist(sp_tr = sp_tr_fruits,             # Species x Traits data frame
                           tr_cat       = sp_tr_cat_fruits,         # Traits informations
...
* `check.input` is a recurrent argument in `mFD`. It defines whether
inputs should be checked before computation or not. Possible error messages will
thus be more understandable for the user than R error messages (**Recommendation:**
set it as `TRUE`.
...
## 6.3. Functional beta diversity indices based on multidimensional space
...
```{r, results = "hide"}
beta_plot_fruits <- mFD::beta.multidim.plot(output_beta.fd.multidim = beta_fd_indices_fruits,
