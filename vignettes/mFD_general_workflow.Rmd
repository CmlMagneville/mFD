...
based on Functional
Entities](https://cmlmagneville.github.io/mFD_shared/articles/How_to_deal_with_Functional_Entities.html)
...
data**:
  ...
  * if **all traits are continuous** use the **Euclidean distance** (`metric =
  ...
  Only Continuous
  Traits](https://cmlmagneville.github.io/mFD_shared/articles/Continuous_traits_framework.html)
...
traits, use
[`gawdis`](https://cran.r-project.org/web/packages/gawdis/index.html) package
instead)
...
We can thus see that **PC1** is mostly driven by *Size* (large fruits on the
right) and *Plant Type* (forb & shrub on the left vs tree & vine on the right),
with weaker influence of *Climate* (eta2 < 0.25) and *Use of fruits* (r2 < 0.4).
Then, **PC2** is mostly driven by *Plant type* (vine on the left, shrubs on the
right) and *Use as jam* (high on the right). **PC3** is driven by *Climate*
(tropical on the right), *Seed* (pit on the right) and *Sugar Content* (high on
the right). And finally **PC4** is driven by only one trait: *Plant Type* with
forbs on the left and vines on the right.
...
alpha_fd_indices_fruits <- mFD::alpha.fd.multidim(
  sp_fxes_coord    = sp_faxes_coord_fruits[, c("PC1", "PC2", "PC3", "PC4")],
...
plots_alpha <- mFD::alpha.multidim.plot(
  ...
  asb_sp_w              = baskets_fruits_weights,
  ind_vect              = c("fdis", "fnnd", "feve", "fric", "fdiv", 
                            "fori", "fspe"),
...
* `details_list` is one of the output of the `mFD::alpha.fd.multidim()` function
and it can be retrieved as shown above.
...
* `fd_ind_values` is one of the output of the `mFD::alpha.fd.multidim()`
function and it can be retrieved as shown above.
...
* `FEve` representation: traits represent the Minimum Spanning Tree linking
