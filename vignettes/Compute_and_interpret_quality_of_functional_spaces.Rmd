...
The dataset used to illustrate this tutorial is *a fruit dataset* based on 25 types of fruits. Each fruit is characterized by 5 traits summarized in the following table:
...
💡 *This dataset is a subset of the dataset used in the [mFD_general_workflow] tutorial <https://github.com/CmlMagneville/mFD/docs/articles/mFD_general_workflow.htlm> to keep only non-fuzzy traits*
...
These traits are summed up in the following dataframe (details: [mFD_general_workflow] tutorial <https://github.com/CmlMagneville/mFD/docs/articles/mFD_general_workflow.htlm>):
 ...
 * blackberry shares 3 traits values with currant and these 2 species have close
 values for other 2 traits wich explains teh low distance (< 0.1)
...
on a PCoA as well as an UPGMA dendrogram using a`mFD::quality.fspaces` function.
Then, we compute 4 quality metrics *(= all combinations of deviation weighting
and distance scaling )* (details: [mFD_general_workflow] tutorial
...
some species pairs are closer in those spaces while they have close trait
...
the trait-based distances** (top-left panel fo the above figure), particularly
...
times lower than distance between each of them and cherry. Euclidean distances
in the 4D space (pcoa_4d) are very similar to those Gower distance, with only a
slight overestimation. Meanwhile, on the UPGMA dendrogram, lime is as distant to
lemon than to the cherry and lemon is even closer to the cherry than to the
lime. This is an illustration of the usual bias of **dendrogram that
overestimates distance between some pairs of species having actually similar
trait values**.
...
```{r, warning = FALSE, out.width = "600px"}
library(tidyr)
library(dplyr)
library(ggplot2)
quality_fspaces_fruits$details_fspaces$pairsp_fspaces_dist %>%
...
litchi, banana and mango is 0.56 while trait-based Gower distance with those 22
fruits varied by a two-fold magnitude from 0.42 (water melon) to 0.84 (currant).
...
`ape::pcoa` function used in the `mFD::quality_fspaces` function.
...
Raw Gower distance between blackberry and banana is twice higher than distance
between blackberry and cherry and 10 times higher than distance between
blackberry and currant because of the high slope of the square-root function
(close to 0). Square-root distance between blackberry and banana differs by a
1.5 and 3-fold factor to distance between blackberry and cherry and currant,
respectively.
...
```{r}
# compute quality metrics with square-root transformated distances:
...
The inaccuracy (measured with *mAD* metric) decreases with the number of axes
down to 0.
...
If we now compute the actual mean absolute deviation between Gower and Euclidean
...
We notice that the mAD = 0.212: inaccuracy is much higher than the worst space
...
of traits with the same values (0, 1 or2)
