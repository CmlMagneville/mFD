...
```{r, echo = FALSE}
sp_tr_cat <- data.frame(names(sp_tr_fruits), c("O","N","O","O","Q"), c(NA, NA, NA, NA, NA))
...
```{r, echo = FALSE}
dist_fruits <- mFD::funct.dist(sp_tr = sp_tr_fruits, tr_cat = sp_tr_cat, dist_metric = "classical_gower", scaling = 'noscale', stop_if_NA = TRUE)
summary(dist_fruits)
```
...
```{r, echo = FALSE}
sp_tr_fruits[c("cherry","lime","lemon"), ]
```
