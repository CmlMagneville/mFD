...
categorical or ordinal traits. It is particularly useful when using large data
sets with "functionally similar" species. FEs also allow to understand the links
between functional diversity and ecological processes as redundant species that
are supposed to have similar ecological roles are clustered in this method.
...
[mFD_general_workflow](https://github.com/CmlMagneville/mFD/docs/articles/mFD_general_workflow.htlm)
...
<br />
   ...
   * the matrix summarizing assemblages:
...
<br />
  ...
  * the dataframe summarizing traits categories (for details: [mFD_general_workflow](https://github.com/CmlMagneville/mFD/docs/articles/mFD_general_workflow.htlm)) 
...
# 4 - Compute FEs coordinates, build functional space, compute & plot alpha and
beta functional indices
...
workflow is the same as the one listed in [mFD_general_workflow](https://github.com/CmlMagneville/mFD/docs/articles/mFD_general_workflow.htlm)
...
However, `mFD` allows to compute functional indices based on FEs following the
...
`mFD::alpha.fd.fe()` function. It computes:
  ...
  * *Functional Overredundancy* that reflects the proportion of species in excess
  in species-rich FE *ie* it represents the percentage of species that fill
  functional entities above the mean level of functional redundancy
...
💡 For this step, species do not have to be gathered into FEs, the gathering is
coded within the `mFD::alpha.fd.fe()` function
...
This function takes as inputs:
  ...
  * *ind_vect* a vector containing the names of the indices to plot. FOred and
  FRed are computed together using the 'red' character string and FVuln is
  computed by using the "fvuln" character string
  * *sp_fe* a vector containing for each species, the FE it belongs to. It is
  retrieved through the `mFD::sp.to.fe()` function
  * *fd_fe_values* a dataframe with the indices values for each assemblage
  retrieved through the `mFD::alpha.fd.fe()` function
  * *fe_nb_sp* a vector containing the number of species per FE. It can
  retrieved through the `mFD::sp.to.fe()` function
  * color and size arguments to personalize the plot. To have more information
  about it, read the `mFD::alpha.fd.fe.plot()` help file
  * *check_input* is a logical value reflecting whether inputs should be checked
  or not. Possible error messages will thus be more understandable for the user
  than R error messages 💡 Recommendation: set it as `TRUE`
