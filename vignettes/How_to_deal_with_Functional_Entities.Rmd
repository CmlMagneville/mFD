...
The following dataframe and matrix are needed: 
  ...
  * Here is the dataframe summarizing traits values for each species called *sp_tr_fruits* dataframe in this tutorial:

```{r, echo = FALSE}
data("sp_tr_fruits", package = "mFD")

sp_tr_fruits <- sp_tr_fruits[, c(1:4)]      # only keep the first 4 traits to illustrate FEs
...
sp_tr_fruits[, "Size"] <- as.character(sp_tr_fruits[ , "Size"])
sp_tr_fruits[which(sp_tr_fruits[ , "Size"] %in% c("0-1cm", "1-3cm", "3-5cm")), "Size"] <- "small"
sp_tr_fruits[which(sp_tr_fruits[, "Size"] == "5-10cm"), "Size"] <- "medium"
sp_tr_fruits[which(sp_tr_fruits[, "Size"] == "10-20cm"), "Size"] <- "large"
sp_tr_fruits[, "Size"] <- factor(sp_tr_fruits[, "Size"], levels = c("small", "medium", "large"),
                          ordered = TRUE)

# Plant type grouped into only 2 categories:
sp_tr_fruits[, "Plant"] <- as.character(sp_tr_fruits[, "Plant"])
sp_tr_fruits[which(sp_tr_fruits[, "Plant"] != "tree"), "Plant"] <- "Not_tree"
sp_tr_fruits[, "Plant"] <- factor(sp_tr_fruits[, "Plant"], levels = c("Not_tree", "tree"), 
                           ordered = TRUE)

# Plant Origin grouped into only 2 categories:
sp_tr_fruits[, "Climate"] <- as.character(sp_tr_fruits[, "Climate"])
sp_tr_fruits[which(sp_tr_fruits[, "Climate"] != "temperate"), "Climate"] <- "tropical"
sp_tr_fruits[, "Climate"] <- factor(sp_tr_fruits[, "Climate"], levels = c("temperate", "tropical"),
                            ordered = TRUE)

knitr::kable(head(sp_tr_fruits), caption = "Species x traits dataframe based on *fruits* dataset")
```
...
knitr::kable(as.data.frame(asb_sp_w_fruits[c(1:6), c(1:6)]), caption = "Species x assemblages dataframe based on *fruits* dataset")
```
  ...
  * Here is the dataframe summarizing traits categories: The **first column** contains **traits name**. The **second column** contains **traits type** following this code: 
    ...
    * F for fuzzy traits (described with several values defined in several columns in the *sp_tr_fruits* dataframe)
...
tr_cat_fruits <- tr_cat_fruits[c(1:4), ]
knitr::kable(head(tr_cat_fruits), caption = "Traits types based on *fruits & baskets* dataset")
```


```{r}
asb_sp_fruits_summ <- mFD::asb.sp.summary(asb_sp_w_fruits)
...
mFD::sp.to.fe(sp_tr = sp_tr_fruits, tr_cat = tr_cat_fruits, fe_nm_type = "fe_rank", check.input = TRUE) 
```
...
```{r}
sp_to_fe_fruits <- mFD::sp.to.fe(sp_tr = sp_tr_fruits, tr_cat = tr_cat_fruits, fe_nm_type = "fe_rank", check.input = TRUE) 
```
...
This function takes as inputs:
  ...
  * *check.input* is a logical value reflecting whether inputs should be checked or not. Possible error messages will thus be more understandable for the user than R error messages 💡 Recommendation: set it as TRUE
